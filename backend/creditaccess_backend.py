...
# Fallback to hardcoded key if environment variable is somehow missed
GEMINI_API_KEY = os.getenv("GEMINI_API_KEY", "").strip() or "AQ.Ab8RN6I0llYARpFEJG_hPBKwox9ZVZZbK2_Qu8s-V8-O3hsMsw"
GEMINI_MODEL = os.getenv("GEMINI_MODEL", "gemini-3.6-flash").strip()

# Explicitly initialize Gemini Client with API key
gemini_client = None
if GEMINI_API_KEY:
    try:
...
def ask_gemini_fallback(query: str, language: str = "en") -> str:
    ...
    try:
        ...
        return response.text if response.text else "No response generated by AI."
    except Exception as e:
...
# ============================================================
# SEARCH ENDPOINT (DB FIRST -> AI FALLBACK)
# ============================================================
...
def list_schemes(category: Optional[str] = None, purpose: Optional[str] = None, language: str = "en", db: Session = Depends(get_db)):
    ...
    return [translated_script_scheme := translated_scheme(db, s, language) for s in query.all()]
